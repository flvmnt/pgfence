...
ALTER TABLE users ADD COLUMN x int;`;
  ...
  it('should set correct statementIndex on policy violations', async () => {
    const results = await analyze([fixture('concurrent-in-tx.sql')], defaultConfig);
    const violations = results[0].policyViolations;
    const concurrentInTx = violations.find((v) => v.ruleId === 'concurrent-in-transaction');
    expect(concurrentInTx).toBeDefined();
    // statementIndex should point to the CONCURRENTLY statement, not other statements
    expect(concurrentInTx!.statementIndex).toBeDefined();
    expect(typeof concurrentInTx!.statementIndex).toBe('number');
...
describe('Plugin system', () => {
  ...
  it('should detect char(n) and character(n) column types (ban-char-field)', async () => {
    const results = await analyze([fixture('new-rules.sql')], defaultConfig);
    const checks = results[0].checks.filter((c) => c.ruleId === 'ban-char-field');
    expect(checks.length).toBeGreaterThanOrEqual(2);
    expect(checks[0].appliesToNewTables).toBe(true);
  });

  it('should detect serial, bigserial, and smallserial (prefer-identity)', async () => {
    const results = await analyze([fixture('new-rules.sql')], defaultConfig);
    const checks = results[0].checks.filter((c) => c.ruleId === 'prefer-identity');
    expect(checks.length).toBeGreaterThanOrEqual(3);
    const messages = checks.map((c) => c.message);
    expect(messages.some((m) => m.includes('bigint GENERATED BY DEFAULT AS IDENTITY'))).toBe(true);
    expect(messages.some((m) => m.includes('smallint GENERATED BY DEFAULT AS IDENTITY'))).toBe(true);
  });

  it('should detect DROP DATABASE and DROP DATABASE IF EXISTS (drop-database)', async () => {
    const results = await analyze([fixture('new-rules.sql')], defaultConfig);
    const checks = results[0].checks.filter((c) => c.ruleId === 'drop-database');
    expect(checks.length).toBe(2);
    expect(checks[0].risk).toBe(RiskLevel.CRITICAL);
    expect(checks[1].message).toContain('staging');
  });

  it('should detect ALTER DOMAIN ADD CONSTRAINT with SHARE lock (ban-alter-domain-add-constraint)', async () => {
    const results = await analyze([fixture('new-rules.sql')], defaultConfig);
    const check = results[0].checks.find((c) => c.ruleId === 'ban-alter-domain-add-constraint');
    expect(check).toBeDefined();
    expect(check!.risk).toBe(RiskLevel.HIGH);
    expect(check!.lockMode).toBe(LockMode.SHARE);
    expect(check!.message).toContain('blocking writes');
  });

  it('should detect CREATE DOMAIN with constraint (ban-create-domain-with-constraint)', async () => {
    const results = await analyze([fixture('new-rules.sql')], defaultConfig);
    const checks = results[0].checks.filter((c) => c.ruleId === 'ban-create-domain-with-constraint');
    expect(checks.length).toBe(1);
    expect(checks[0].risk).toBe(RiskLevel.LOW);
    expect(checks[0].lockMode).toBe(LockMode.ACCESS_SHARE);
  });

  it('should NOT trigger on ALTER DOMAIN DROP CONSTRAINT or CREATE DOMAIN without constraint', async () => {
    const results = await analyze([fixture('new-rules.sql')], defaultConfig);
    const checks = results[0].checks;
    // ALTER DOMAIN DROP CONSTRAINT should not fire ban-alter-domain-add-constraint
    const dropConstraintChecks = checks.filter(
      (c) => c.ruleId === 'ban-alter-domain-add-constraint' && c.statement.includes('DROP CONSTRAINT'),
    );
    expect(dropConstraintChecks.length).toBe(0);
    // CREATE DOMAIN without constraint should not fire ban-create-domain-with-constraint for my_int
    const domainNoConstraint = checks.filter(
      (c) => c.ruleId === 'ban-create-domain-with-constraint' && c.statement.includes('my_int'),
    );
    expect(domainNoConstraint.length).toBe(0);
  });
