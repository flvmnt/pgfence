 ...
 * - char/character/bpchar instead of text (padded, wastes space)
 * - timestamp without time zone instead of timestamptz (timezone bugs)
 * - SERIAL/BIGSERIAL instead of IDENTITY columns (PG10+ best practice)
 *
...
export function checkBestPractices(stmt: ParsedStatement): CheckResult[] {
  ...
  for (const { colDef, tableName } of columns) {
    ...
    // ban-char-field: char/character/bpchar -> text or varchar
    if (isCharType(typeName)) {
      results.push({
        ...
        blocks: getBlockedOperations(LockMode.ACCESS_EXCLUSIVE),
        risk: RiskLevel.LOW,
        message: `Column "${colDef.colname}" uses ${typeName}: use text instead. char(n) pads values with spaces, wastes storage, and changing the length later requires ACCESS EXCLUSIVE lock + table rewrite`,
        ruleId: 'ban-char-field',
        appliesToNewTables: true,
      });
    }
...
function isCharType(typeName: string): boolean {
  return ['char', 'character', 'bpchar'].includes(typeName);
}

function isSerialType(typeName: string): boolean {
  return ['serial', 'serial2', 'serial4', 'serial8', 'smallserial', 'bigserial'].includes(typeName);
}

function serialToIdentity(typeName: string): string {
  switch (typeName) {
    case 'serial':
    case 'serial4':
